...
def load_data(
    ...
    if train:
        return DataLoader(
            dataset=OieDataset(data_path, max_len, tokenizer_config),
        ...
        return DataLoader(
            dataset=OieEvalDataset(data_path, max_len, tokenizer_config),
            batch_size=batch_size,
            num_workers=4,
            pin_memory=True,
        )
...
class OieEvalDataset(Dataset):
    def __init__(
        self, data_path, max_len, tokenizer_config="bert-base-multilingual-cased"
    ):
