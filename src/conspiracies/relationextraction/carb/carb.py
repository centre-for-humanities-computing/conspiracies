...
class Benchmark:
    ...
    def compare(
        self, predicted, matchingFunc, output_fn, error_file=None, binary=False
    ):
        ...
        try:
            ...
            optimal = (
                prec_scores[optimal_idx],
                rec_scores[optimal_idx],
                f1s[optimal_idx],
            )
        except ValueError:
        ...
        if len(f1s) > 0:
            rec_prec_dict = {
                rec: prec for rec, prec in zip(temp_rec_scores, temp_prec_scores)
            }
            ...
            return np.round(auc(temp_rec_scores, temp_prec_scores), 5), np.round(
                optimal, 5
            )
        else:
            # When there is no prediction
...
if __name__ == "__main__":
    ...
    auc, optimal_f1_point = b.compare(
