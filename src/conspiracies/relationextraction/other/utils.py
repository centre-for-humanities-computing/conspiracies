...
def get_train_modules(model, lr, total_steps, warmup_steps):
...
class SummaryManager:
    ...
    def save_results(self, results):
        self.result_df = pd.read_csv(
            os.path.join(self.config.save_path, "train_results.csv")
        )
    ...
    def save_df(self):
        self.result_df.to_csv(
            os.path.join(self.config.save_path, "train_results.csv"), index=False
        )
