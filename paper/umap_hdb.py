...
def load_triplets(
    ...
    subjects = [triplet[0] for triplet in filtered_triplets]
    predicates = [triplet[1] for triplet in filtered_triplets]
    objects = [triplet[2] for triplet in filtered_triplets]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--event",
        type=str,
        help="Event to cluster. Triplets must be extracted usin extract_triplets_newspapers.py",
    )
    parser.add_argument(
        "-emb",
        "--embedding_model",
    ...
    path = os.path.join("extracted_triplets_papers", args.event, "triplets.txt")
    main(
        path,
