...
for i in range(2): # This should really be automized
    ...
    first_file = os.path.join(root_path, 'predictions', f'template{first}_gpt_outputs_{i}.json')
    ...
    for d in first_data:
        ...
        for n in template_numbers[1:]:
            file = os.path.join(root_path, 'predictions', f'template{n}_gpt_outputs_{i}.json')
