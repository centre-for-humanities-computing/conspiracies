...
from create_templates import prompt_template_1, prompt_template_2, prompt_template_3, prompt_template_4
...
with open(os.path.join(root_path, 'tagged', 'tagged_tweets_with_features.json'), "r", encoding="utf8") as f:
    data = json.load(f)
...
for i in range(2):
    ...
    with open(os.path.join(root_path, 'predictions', f"examples_set_{i}.json"), "w") as outfile:
    ...
    for key, value in dict_functions.items():
        ...
        for tweet in target_tweets:
            template = value(examples=examples, target_tweet=tweet, introduction="Extract semantic triples from the following tweets and put them in the form (Subject)(VerbPhrase)(Arg):")
            print("Template used:\n\n")
            print(template)
            print("\n\n")
        ...
        with open(os.path.join(root_path, 'predictions', f"{key}_gpt_outputs_{i}.json"), "w") as outfile:
