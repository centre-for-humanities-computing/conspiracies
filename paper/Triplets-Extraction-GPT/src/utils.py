...
def sanity_check_triplets(triplets:List[List[str]], counter:Optional[Counter]=None)->List[List[str]]:
    """Sanity check for a given set of triplets.
    Checks if triplets indicate template is trying to start new table,
    if triplet start with a new tweet (not the target tweet)
    and if triplets are exactly three elements long.
    It removes such none-sane triplets from the list
...
def write_triplets(
    ...
    if not checked_triplets:
        ...
        for i, triplet in enumerate(checked_triplets):
            ...
            if i == 0:
                triplet_string = f"|**{source}**|{triplet[0]}|{triplet[1]}|{triplet[2]}|\n"
            else:
