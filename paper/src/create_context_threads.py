...
def context_window_thread(tweets, context_len):
    ...
    def find_context(tweet, conversation, context_length=context_len):
        ...
        context = find_context(
            conversation[reply_to_id], conversation, context_length=context_length - 1
        )
...
if __name__ == "__main__":
    ...
    with open(
        os.path.join(
            f"TESTnew_tweet_threads_{datetime.datetime.strptime(args.start_date, '%Y-%m-%d').date() }_{datetime.datetime.strptime(args.start_date, '%Y-%m-%d').date() + datetime.timedelta(days=args.extra_days-1)}.ndjson"
        ),
        "w",
    ) as f:
