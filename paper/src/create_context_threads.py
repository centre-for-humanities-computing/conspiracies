...
def ndjson_gen(filepath: str):
    """
    Creates a generator for ndjson files and yields the tweets without retweets
    """
...
def context_window_thread(tweets, context_len):
    ...
    def find_context(tweet, conversation, context_length=context_len):
        ...
        if "in_reply_to_user_id" not in tweet or context_length == 0 or "referenced_tweets" not in tweet:
            return [tweet]
        ...
        context = find_context(conversation[reply_to_id], conversation, context_length=context_length - 1)
...
if __name__ == "__main__":
    ...
    with open(os.path.join(f"TESTnew_tweet_threads_{datetime.datetime.strptime(args.start_date, '%Y-%m-%d').date() }_{datetime.datetime.strptime(args.start_date, '%Y-%m-%d').date() + datetime.timedelta(days=args.extra_days-1)}.ndjson"), "w") as f:
